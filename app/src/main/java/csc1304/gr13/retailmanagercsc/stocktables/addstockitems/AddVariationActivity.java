...
public class AddVariationActivity extends FragmentActivity implements MenuFragment.OnTransactionItemClickListener {
    ...
    private AddVariationDBfrg addVariationDBfrg;

    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_generic_fragment);

        addVariationDBfrg = new AddVariationDBfrg();
        tranTitle = findViewById(R.id.trans_name);
        tranTitle.setText("ADD PRODUCT VARIATION");
        this.mViewPager = findViewById(R.id.idtype_menu_pager);
        this.fragmentList.add(addVariationDBfrg);
    ...
    private void startTimeTask() {
        ...
        new Thread() {
            public void run() {
                while (!AddVariationActivity.this.isEndTimeout) {
                    if (System.currentTimeMillis() - AddVariationActivity.this.startTime > ((long) (AddVariationActivity.this.timeout * 1000))) {
                        AddVariationActivity.this.finish();
                    }
                    try {
                        Thread.sleep(100);
