...
//    public static final String COL_P_SIZE = "size";
    ...
    public Long storeProductInfo(ProductDatabaseModel product){

        this.Open();
        ContentValues contentValues = new ContentValues();
        //contentValues.put(dbHelper.COL_P_ID,product.getProductId());
        ...
        contentValues.put(dbHelper.COL_P_CATEGORY,product.getProductCategory());

        long id = database.insert(dbHelper.TABLE_P_NAME,null,contentValues);

        this.Close();
        if(id > 0){
            Log.d(TAG, "Product: "+id+"------------ new Product inserted");
            return id;
        }else {
...
public ArrayList<ProductDatabaseModel> getProducts() {
        ...
        try {
            ...
            if (stockTableCursor.getCount() > 0) {
                for (int i = 0; stockTableCursor.getCount() > i; i++) {
                    ...
                    Cursor cursor = database.query(dbHelper.TABLE_P_NAME, null, dbHelper.COL_P_ID+"=?", new String[] {stockTableCursor.getString(stockTableCursor.getColumnIndex(dbHelper.COL_ST_PRODUCT_ID))}, null, null, null);
                    cursor.moveToFirst();

                    if (cursor.getCount() > 0) {
                        products.add(new ProductDatabaseModel(
                                cursor.getInt(cursor.getColumnIndex(dbHelper.COL_P_ID)),
                                cursor.getString(cursor.getColumnIndex(dbHelper.COL_P_NAME)),
                                cursor.getString(cursor.getColumnIndex(dbHelper.COL_P_CODE)),
                                cursor.getString(cursor.getColumnIndex(dbHelper.COL_P_PRICE)),
                                cursor.getString(cursor.getColumnIndex(dbHelper.COL_P_SELL_PRICE)),
                                cursor.getString(cursor.getColumnIndex(dbHelper.COL_P_UNIT)),
                                cursor.getString(cursor.getColumnIndex(dbHelper.COL_P_BRAND)),
                                cursor.getString(cursor.getColumnIndex(dbHelper.COL_P_SIZE)),
                                cursor.getColumnName(cursor.getColumnIndex(dbHelper.COL_P_CATEGORY)),
    ...
    public ProductListModel getProductDetails(String productId) {

        this.Open();

        ProductListModel product;
        try {
            Cursor productCursor = database.query(dbHelper.TABLE_P_NAME, null, dbHelper.COL_P_ID + "=?", new String[]{productId}, null, null, null);
            Cursor stockCursor = database.query(dbHelper.TABLE_ST_NAME, null, dbHelper.COL_ST_PRODUCT_ID + "=?", new String[]{productId}, null, null, null);
            ...
            if (productCursor.getCount() > 0 && stockCursor.getCount() > 0) {
                ...
                String sell_price = productCursor.getString(productCursor.getColumnIndex(dbHelper.COL_P_SELL_PRICE));
                String product_price = productCursor.getString(productCursor.getColumnIndex(dbHelper.COL_P_PRICE));

                if(sell_price != null && sell_price.length() != 0 && !sell_price.equalsIgnoreCase(product_price)){
                    product = new ProductListModel(
                            productCursor.getString(productCursor.getColumnIndex(dbHelper.COL_P_ID)),
                            sell_price,
